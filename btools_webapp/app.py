...
def call_gemini_api(raw_text: str, api_key: str) -> dict:
    ...
    9. **ห้ามพิมพ์ชื่อหมวดหมู่ซ้ำ:** หากหมวดหมู่นั้นคือ Objectives ห้ามสร้างข้อย่อยบรรทัดแรกว่า "วัตถุประสงค์ (Objectives)" ซ้ำอีก หรือหากหมวดหมู่คือ Rationale ห้ามสร้างบรรทัดแรกว่า "หลักการและเหตุผล" ซ้ำ ให้ดึงมาเฉพาะเนื้อหาเท่านั้น
    10. **หัวข้อย่อยในบรรทัดเดียวกัน:** คำที่ดูเหมือนหัวข้อก่อนเริ่มอธิบายเนื้อหาในบรรทัดเดียวกัน (เช่น "Mindset Transformation:", "ข้อควรระวัง:") ให้ทำเป็นตัวหนาเสมอ (เช่น **Mindset Transformation:**) โดยอิงตามต้นฉบับ
    11. **รูปแบบตาราง (Table):** หากต้นฉบับเป็น "ตาราง" ห้ามแปลงเป็นข้อย่อย (Bullet) เด็ดขาด ให้คุณเก็บข้อมูลลงในฟิลด์ `"table"` แทน โดยแยกเป็น `"headers"` และ `"rows"` ให้ถูกต้องตามโครงสร้าง
